...
function fail(message){throw new Error(`v1.56.0.3 runtime/repository audit: ${message}`);}
...
if(pkg.version!=='1.56.0.3')fail(`package version is ${pkg.version}, expected 1.56.0.3`);
...
// must not reappear.
requireToken(adapter,"const VERSION='1.56.0.3';",'map adapter version');
...
requireToken(app,'// v1.56.0.3: OpenLayers is the sole map runtime. Application code talks only','application runtime boundary');
...
const authority='/* v1.56.0.3 — runtime authority boundary.';
...
requireToken(authorityTail,"version:'1.56.0.3'",'runtime authority snapshot');
...
requireToken(mobile,"const VERSION='1.56.0.3';",'mobile controller version');
...
// v1.56.0.3 Processing Toolbox invariants: one declarative registry, one shared
...
requireToken(app,"EditPolygonProjectFormat.createArchive(payload,{appVersion:'1.56.0.3'})",'EPZ save version');
...
console.log('v1.56.0.3 runtime/repository audit passed. OpenLayers authority remains intact and the declarative browser-local Processing Toolbox is wired through one validated worker lifecycle.');

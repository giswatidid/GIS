...
const processingRegistry=read('docs/assets/gis-processing-registry.js');
const processingCore=read('docs/assets/gis-processing-core.js');
const processingUi=read('docs/assets/gis-processing.js');
const processingWorker=read('docs/assets/gis-processing-worker.js');
const processingEngine=read('docs/assets/gis-processing-engine.js');
const spatialCore=read('docs/assets/gis-spatial-core.js');
const pkg=JSON.parse(read('package.json'));
const RELEASE_KEY='20260817-v1561-geometry-code-inspector-final';
...
function fail(message){throw new Error(`v1.56.1 runtime/repository audit: ${message}`);}
...
if(pkg.version!=='1.56.1')fail(`package version is ${pkg.version}, expected 1.56.1`);
...
// must not reappear.
requireToken(adapter,"const VERSION='1.56.1';",'map adapter version');
...
requireToken(app,'// v1.56.1: OpenLayers is the sole map runtime. Application code talks only','application runtime boundary');
...
const authority='/* v1.56.1 — runtime authority boundary.';
...
requireToken(authorityTail,"version:'1.56.1'",'runtime authority snapshot');
...
requireToken(mobile,"const VERSION='1.56.1';",'mobile controller version');
...
// v1.56.1 Processing Toolbox invariants: the full declarative catalogue shares
// one generic request/result contract, one worker lifecycle, shared GEOS/spatial
// engines, explicit scope semantics, and commit-after-result project mutation.
for(const token of ["id:'buffer'","id:'union'","id:'intersection'","id:'difference'","id:'symmetric-difference'","id:'clip'","id:'dissolve'","id:'multipart-to-singlepart'","id:'select-by-attribute'","id:'select-by-location'","id:'nearest-feature'","id:'join-by-location'","id:'fix-geometries'","id:'snap'","id:'simplify'","id:'densify'"])requireToken(processingRegistry,token,'processing registry');
for(const token of ["const DEFAULT_SCOPES=Object.freeze(['all','filtered','selected'])",'function preflight(','function createProvenance(','function resolveProcessingCrs('])requireToken(processingCore,token,'processing core');
for(const token of ["scope==='selected'?selected.has(feature.id)","scope==='filtered'?!feature.filtered:true"])requireToken(processingCore,token,'processing scope semantics');
forbidToken(processingCore,'feature.visible','processing membership must ignore presentation visibility');
for(const token of ['Find a tool','Layer visibility does not change processing membership.','Run and create layer','Run and modify layer','Processing completed'])requireToken(processingUi,token,'Processing Toolbox UI');
for(const token of ['gis-spatial-core.js?v=${KEY}','gis-geos-adapter.js?v=${KEY}','gis-processing-engine.js?v=${KEY}','EditPolygonGISProcessingEngine.execute'])requireToken(processingWorker,token,'shared processing worker execution');
for(const token of ['function geosOverlay(','function dissolveGeometry(','function executeSpatial(','function applySelectionMode('])requireToken(processingEngine,token,'shared processing engine');
for(const token of ['function buildSpatialIndex(','function relation(','function nearestFeature(','function aggregate('])requireToken(spatialCore,token,'shared spatial engine');
requireToken(app,"new Worker(`assets/gis-processing-worker.js?v=${PROCESSING_KEY}`)",'processing worker cache key');
for(const token of ['previewProcessingRequest','runProcessingRequest','cancelProcessing','createProcessingLayer','modifyProcessingSource','applyProcessingSelection'])requireToken(app,token,'processing application bridge');
...
for(const asset of ['gis-processing-registry.js','gis-processing-core.js','gis-processing-engine.js','gis-spatial-core.js','gis-processing.js','gis-processing.css'])requireToken(html,`${asset}?v=${RELEASE_KEY}`,`processing deployment ${asset}`);
...
requireToken(app,"EditPolygonProjectFormat.createArchive(payload,{appVersion:'1.56.1'})",'EPZ save version');
...
console.log('v1.56.1 runtime/repository audit passed. OpenLayers authority remains intact and the declarative browser-local Processing Toolbox is wired through one validated worker lifecycle.');

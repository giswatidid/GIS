...
const RELEASE_KEY='20260811-v15574-single-sketch-authority';
...
function fail(message){throw new Error(`v1.55.7.4 runtime/repository audit: ${message}`);}
...
if(pkg.version!=='1.55.7.4')fail(`package version is ${pkg.version}, expected 1.55.7.4`);
...
// must not reappear.
requireToken(adapter,"const VERSION='1.55.7.4';",'map adapter version');
...
requireToken(app,'// v1.55.7.4: OpenLayers is the sole map runtime. Application code talks only','application runtime boundary');
...
const authority='/* v1.55.7.4 — runtime authority boundary.';
...
requireToken(authorityTail,"version:'1.55.7.4'",'runtime authority snapshot');
...
requireToken(mobile,"const VERSION='1.55.7.4';",'mobile controller version');
...
requireToken(app,"EditPolygonProjectFormat.createArchive(payload,{appVersion:'1.55.7.4'})",'EPZ save version');
...
console.log('v1.55.7.4 runtime/repository audit passed. Draw-time navigation is adapter-owned and unfinished sketch geometry has one OpenLayers transient-vector authority; mobile parity, lossless .epz persistence and authoritative rendering remain intact.');

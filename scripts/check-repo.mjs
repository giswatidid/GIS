...
if(html.indexOf('leaflet@1.9.4/dist/leaflet.js')>html.indexOf('editpolygon-map-adapter.js'))throw new Error('Leaflet must load before the v1.55.1 dual-engine adapter.');
if(!html.includes('cdn.jsdelivr.net/npm/ol@v10.9.0/dist/ol.js'))throw new Error('OpenLayers parity build is not wired into the page.');
...
for(const token of ['createRuntime','createLeafletRuntime','createOpenLayersRuntime','createEditableVectorLayer','createTileLayer','createWmsLayer','lonLatToPixel','pixelToLonLat','fitExtent','setPanEnabled','setDoubleClickZoomEnabled','nativePanLooksActive','recoverNativePan'])if(!mapAdapter.includes(token))throw new Error('Map adapter missing '+token);

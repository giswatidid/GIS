...
for(const token of ['createRuntime','createLeafletRuntime','createOpenLayersRuntime','createEditableVectorLayer','createTileLayer','createWmsLayer','createGeoJsonLayer','createStaticImageLayer','setDisplayLayerOpacity','setDisplayLayerVisible','setDisplayLayerZIndex','setGeoJsonLayerStyle','lonLatToPixel','pixelToLonLat','fitExtent','setPanEnabled','setDoubleClickZoomEnabled','nativePanLooksActive','recoverNativePan'])if(!mapAdapter.includes(token))throw new Error('Map adapter missing '+token);
for(const token of ['MAP_RUNTIME','window.EditPolygonMap=MAP_RUNTIME','createRuntime','requestedEngine(location.search)','buildOpenLayersCachedLayer'])if(!app.includes(token))throw new Error('Application missing map abstraction '+token);
for(const token of ['window.syncReferenceMapLayers=syncReferenceMapLayers','MAP_RUNTIME.createGeoJsonLayer','MAP_RUNTIME.createStaticImageLayer','referenceLayerZIndex'])if(!app.includes(token))throw new Error('Application missing native reference integration '+token);
for(const file of ['ARCHITECTURE.md','CHANGELOG.md','V1.55.3_AUDIT.md','V1.55.3_CHANGED_FILES.md','CRS_VALIDATION.md'])if(!fs.existsSync(file))throw new Error('Missing consolidated repository document '+file);
for(const token of ['editpolygon-leaflet-compat','__peGetLeafletMap','__polygonEditorLeafletMap','getLegacyMap','syncLegacy'])if(app.includes(token)||mapAdapter.includes(token))throw new Error('Obsolete OpenLayers compatibility-map token remains: '+token);

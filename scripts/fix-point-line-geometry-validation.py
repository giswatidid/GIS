...
unit=replace_once(unit,"const app=fs.readFileSync(new URL('../docs/assets/editpolygon-app.js',import.meta.url),'utf8');\n","const app=fs.readFileSync(new URL('../docs/assets/editpolygon-app.js',import.meta.url),'utf8');\nconst geometryHealthCore=fs.readFileSync(new URL('../docs/assets/gis-geometry-health-core.js',import.meta.url),'utf8');\n",'unit Geometry Health fixture')
unit=replace_once(unit,"""    document:{readyState:'loading',addEventListener(){},getElementById(){return null;}},
...
""",'unit hostile legacy validator')
unit=replace_once(unit,"""  vm.createContext(sandbox);
...
""","""  vm.createContext(sandbox);
  ...
  vm.runInContext(moduleCode,sandbox,{filename:'point-line-geometry-code-v2.js'});
""",'unit shared Geometry Health load')
unit=replace_once(unit,"""  assert.match(moduleCode,/data-plgce-section=\"code\"/);
...
""","""  assert.match(moduleCode,/data-plgce-section=\"code\"/);
  ...
  assert.match(app,/__pointLineGeometryCodeV2\?\.ensureNow\?\.\(\)/);
""",'unit validation architecture assertions')
anchor="""test('LineString and MultiLineString code validates without changing geometry family',()=>{
...
"""
unit=replace_once(unit,anchor,addition,'unit type-aware validation regression')
UNIT.write_text(unit,encoding='utf-8')

browser=BROWSER.read_text(encoding='utf-8')
browser=replace_once(browser,"""    page.locator('.plgce-code').fill('{\"type\":\"Point\",\"coordinates\":[154,-28]}')
...
browser=replace_once(browser,"""    page.locator('.plgce-code').fill('{\"type\":\"LineString\",\"coordinates\":[[153,-27],[155,-29],[156,-30]]}')
    page.locator('[data-plgce=\"apply\"]').click()
...
cache_files=[ROOT/'docs/index.html',ROOT/'docs/assets/editpolygon-app.js',ROOT/'docs/assets/gis-processing-worker.js',ROOT/'scripts/audit-runtime.mjs',ROOT/'tests/release-cache.test.mjs',ROOT/'tests/gis-crs-integration.test.mjs',ROOT/'tests/gis-remote-source-integration.test.mjs',ROOT/'tests/typed-fields-integration.test.mjs',ROOT/'tests/render-performance.test.mjs',ROOT/'tests/processing-toolbox-integration.test.mjs']

...
function fail(message){throw new Error(`v1.56.0.2 binding/architecture audit: ${message}`);}
...
if(duplicateNames.length>198)fail(`duplicate function-binding names grew to ${duplicateNames.length} (v1.56.0.2 ceiling 198)`);
if(extraBindings>371)fail(`extra historical function bindings grew to ${extraBindings} (v1.56.0.2 ceiling 371)`);
...
// v1.56.0.2 keeps the native OpenLayers implementation adapter-owned. The
...
const authorityMarker='/* v1.56.0.2 — runtime authority boundary.';
...
if(!authorityTail.includes("version:'1.56.0.2'"))fail('runtime authority snapshot has the wrong version');
...
console.log(`v1.56.0.2 binding/architecture audit passed: ${bindings.size} named bindings, ${duplicateNames.length} duplicate names, ${extraBindings} extra sites, 0 application engine branches, 0 application native-map calls, 0 native-map escapes. Highest wrapper chains: ${top}.`);

...
test('map adapter publishes one OpenLayers runtime factory and no legacy engine selector',()=>{
  const {context}=openLayersContext(),api=context.EditPolygonMapAdapter;
  assert.equal(api.version,'1.55.6');
  assert.equal(typeof api.createRuntime,'function');
  assert.equal(typeof api.createOpenLayersRuntime,'function');
...
test('transient vector overlays and DOM handles are native map-runtime primitives',()=>{
  ...
  const el=handle.getElement(),pane=target.children.find(child=>child.className==='editpolygon-openlayers-dom-overlays');
...
test('click delivery is native and has no compatibility DOM surface',()=>{
  ...
  runtime.getNativeMap().fire('click',{pixel:[320,210],coordinate:[3200,2100],originalEvent:{}});

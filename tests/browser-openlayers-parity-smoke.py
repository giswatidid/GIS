...
class S{constructor(o={}){this.o=o}} class VS extends S{constructor(o={}){super(o);this.f=[];this.changedCount=0}addFeatures(x){this.f.push(...x)}getFeatureById(id){return this.f.find(x=>x.id===id)||null}changed(){this.changedCount++}}
...
with sync_playwright() as p:
    ...
    result=page.evaluate('''()=>{
      ...
      return {engine:r.engine,bridge:r.parityBridge,center:r.getCenter(),zoom:r.getZoom(),pixel:[p.x,p.y],layers:r.getNativeMap().getLayers().getArray().length,featureCount:vec.__editpolygonFeatureCount,disabled,compat:!!r.getLegacyMap(),liveUpdated,liveCoordinates:liveGeometry.coordinates};
    }''')

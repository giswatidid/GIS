...
with sync_playwright() as p:
    ...
    result=page.evaluate('''async()=>{
      ...
      const wms=r.createWmsLayer({url:'https://example.test/geoserver/wms',layers:'demo',opacity:.6,zIndex:25});
      const wmsInitiallyPresent=r.hasDisplayLayer(wms);r.setDisplayLayerVisible(wms,true);const wmsShownPresent=r.hasDisplayLayer(wms);r.setDisplayLayerVisible(wms,false);const wmsHiddenAbsent=!r.hasDisplayLayer(wms);r.setDisplayLayerVisible(wms,true);

...
test('EPZ archive creation keeps the canonical project JSON intact and adds integrity metadata',async()=>{
  ...
  const result=await format.createArchive(payload,{appVersion:'1.56.0.4'});
  ...
  assert.equal(result.manifest.appVersion,'1.56.0.4');
...
test('EPZ archive loading verifies and returns the exact project payload',async()=>{
  ...
  const created=await format.createArchive(payload,{appVersion:'1.56.0.4'});
...
test('application uses only .epz for project save/open and loads the format module before the app',()=>{
  assert.match(app,/EditPolygonProjectFormat\.createArchive\(payload,\{appVersion:'1\.56\.0\.4'\}\)/);
...
test('opening an EPZ starts a clean, non-undoable editing session',()=>{
  const resetFn=app.match(/function resetProjectHistoryAfterOpen\(\)[\s\S]*?\n}/)?.[0]||'';
  assert.match(resetFn,/project\.history=\[\];[\s\S]*?project\.future=\[\];[\s\S]*?IMAGE\.history=\[\];[\s\S]*?IMAGE\.future=\[\];/);
  assert.match(resetFn,/HISTORY_SESSION\.projectOpenBaseline=true/);
  assert.equal((resetFn.match(/resetProjectHistoryAfterOpen\(\)/g)||[]).length,1,'history reset must not recurse into itself');
  assert.doesNotMatch(app,/pushHistory\(\);\s*restoreCompleteProjectPayload\(imported\.payload/);
  const finalRestore=app.match(/const v140BaseRestoreCompleteProjectPayload=[\s\S]*?window\.restoreCompleteProjectPayload=restoreCompleteProjectPayload;/)?.[0]||'';
  assert.match(finalRestore,/HISTORY_SESSION\.restoreDepth\+\+/);
  assert.match(finalRestore,/finally\s*\{[\s\S]*?resetProjectHistoryAfterOpen\(\);/);
  assert.match(app,/function undo\(\)\{\s*if\(HISTORY_SESSION\.projectOpenBaseline\)return;/);
  assert.match(app,/function redo\(\)\{\s*if\(HISTORY_SESSION\.projectOpenBaseline\)return;/);
  assert.match(app,/function pushHistory\(featureIds=null\)\{[\s\S]*?if\(!HISTORY_SESSION\.restoreDepth\)HISTORY_SESSION\.projectOpenBaseline=false;/);
  // Undo/redo stacks are transient editor state and must not be persisted in project.json.
  const saveFn=app.match(/function createSaveProjectPayload\(\)[\s\S]*?\n}\nasync function saveProject/)?.[0]||'';
  assert.doesNotMatch(saveFn,/history\s*:/);
  assert.doesNotMatch(saveFn,/future\s*:/);
});

...
test('EPZ archive creation keeps the canonical project JSON intact and adds integrity metadata',async()=>{
  ...
  const result=await format.createArchive(payload,{appVersion:'1.56.0.3'});
  ...
  assert.equal(result.manifest.appVersion,'1.56.0.3');
...
test('EPZ archive loading verifies and returns the exact project payload',async()=>{
  ...
  const created=await format.createArchive(payload,{appVersion:'1.56.0.3'});
...
test('application uses only .epz for project save/open and loads the format module before the app',()=>{
  assert.match(app,/EditPolygonProjectFormat\.createArchive\(payload,\{appVersion:'1\.56\.0\.3'\}\)/);
...
test('opening an EPZ starts a clean undo session rather than retaining pre-open history',()=>{
  assert.match(app,/function resetProjectHistoryAfterOpen\(\)[\s\S]*?project\.history=\[\];[\s\S]*?project\.future=\[\];[\s\S]*?IMAGE\.history=\[\];[\s\S]*?IMAGE\.future=\[\];/);
  ...
  assert.match(finalRestore,/resetProjectHistoryAfterOpen\(\);/);

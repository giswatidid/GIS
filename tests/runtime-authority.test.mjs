...
test('v1.55.7.3 publishes one final runtime-authority snapshot at the end of the application scope',()=>{
  const marker='/* v1.55.7.3 — runtime authority boundary.';
  const start=app.indexOf(marker);assert.ok(start>=0);
  const tail=app.slice(start);
  assert.match(tail,/version:'1\.55\.7\.3'/);
...
test('retired pan-recovery drag hook cannot abort startup before Advanced GIS installs',()=>{
  ...
  const authority=app.indexOf('/* v1.55.7.3 — runtime authority boundary.');

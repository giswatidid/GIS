...
test('v1.56.1 publishes one final runtime-authority snapshot at the end of the application scope',()=>{
  const marker='/* v1.56.1 — runtime authority boundary.';
  const start=app.indexOf(marker);assert.ok(start>=0);
  const tail=app.slice(start);
  assert.match(tail,/version:'1\.56\.1'/);
...
test('retired pan-recovery drag hook cannot abort startup before Advanced GIS installs',()=>{
  ...
  const authority=app.indexOf('/* v1.56.1 — runtime authority boundary.');

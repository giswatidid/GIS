...
with sync_playwright() as p:
    ...
    page.evaluate("""() => {
      ...
      window.EditPolygonGISProcessingEngine={execute:async(task,{onProgress})=>{onProgress({stage:'Simplifying',done:1,total:1,percent:80});await new Promise(resolve=>setTimeout(resolve,80));return{kind:'layer',features:[{type:'Feature',id:'a',properties:{name:'A'},geometry:{type:'LineString',coordinates:[[153,-27],[153.4,-27]]}}],summary:{input:1,processed:1,output:1,failed:0},failures:[],processingCrs:'EPSG:32756'};}};
      window.__setCalls=0;window.__addCalls=0;window.__mapHandlers={};window.__overlay=null;window.__runCalls=0;window.__modalCloseClicks=0;
      ...
      window.EditPolygonGIS={getEditableLayers:()=>[layer],getSelection:()=>({ids:[]}),previewProcessingRequest:preflight,runProcessingRequest:async()=>{window.__runCalls++;await new Promise(resolve=>setTimeout(resolve,80));return{kind:'layer',output:{id:'o',name:'Result'},summary:{input:1,processed:1,output:1,failed:0},failures:[]}},cancelProcessing:()=>true};
      document.getElementById('gisDataClose').addEventListener('click',()=>{window.__modalCloseClicks++;document.getElementById('gisDataModal').classList.remove('active');});
    }""")
    ...
    idle_cancel=page.locator('[data-processing-action="cancel"]')
    assert idle_cancel.inner_text()=='Cancel processing' and idle_cancel.is_disabled()
    page.locator('[data-processing-action="preview"]').click()
    running_cancel=page.locator('[data-processing-action="cancel"]')
    assert running_cancel.inner_text()=='Cancel preview' and not running_cancel.is_disabled()
    ...
    assert page.locator('[data-processing-action="close-processing"]').inner_text()=='Close'
    assert page.locator('[data-processing-action="cancel"]').inner_text()=='Cancel processing'
    assert page.locator('[data-processing-action="cancel"]').is_disabled()
    ...
    processing_cancel=page.locator('[data-processing-action="cancel"]')
    assert processing_cancel.inner_text()=='Cancel processing' and not processing_cancel.is_disabled()
    ...
    assert page.evaluate('window.__overlay.items.length')==1
    page.locator('[data-processing-action="close-processing"]').click()
    assert not page.locator('#gisDataModal').evaluate("e=>e.classList.contains('gis-processing-map-preview')")
    assert not page.locator('#gisDataModal').evaluate("e=>e.classList.contains('active')")
    assert page.evaluate('window.__overlay.items.length')==0,'Close must discard the temporary preview overlay'
    assert page.evaluate('window.__modalCloseClicks')==1,'Close must use the existing Data/Processing modal close lifecycle'

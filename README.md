...
**Current application baseline:** v1.55.7
...
This runs repository integration checks, the v1.55.7 single-runtime/performance audit, the binding/architecture no-growth audit and the JavaScript test suite.
...
1. **v1.55.7 — OpenLayers cleanup/performance (current):** transition-era runtime shims are removed, native render churn is reduced, geometry conversion is reused, DOM-overlay refresh is batched and zoom lifecycle delivery is centrally fanned out.

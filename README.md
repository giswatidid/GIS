...
**Current application baseline:** v1.55.4.3
...
The v1.55 map-engine work is a staged migration from Leaflet to OpenLayers. **v1.55.4** established the parity and architecture baseline; **v1.55.4.3** includes the deployed parity corrections found so far. OpenLayers remains opt-in with `?mapEngine=openlayers`, but the editable renderer, rendered hit-testing, selection-highlight refresh, references, GIS services and transient/edit overlays sit behind the `EditPolygonMap` contract. v1.55.4.1 aligned true-circle preview and committed display geometry. v1.55.4.2 makes viewport culling longitude-periodic, so editable vectors remain present after repeated horizontal world wraps, and coordinate-to-pixel conversion targets the equivalent world copy nearest the current view. v1.55.4.3 makes polygon/line drawing use the same continuous-longitude branch across the International Date Line and removes stale Shift-pan/zoom overlay states that could make drawing clicks silently fall through to the map. Leaflet remains the default/reference engine only until the deployed parity checklist passes. See [`ARCHITECTURE.md`](ARCHITECTURE.md), [`QUALITY_BASELINE.md`](QUALITY_BASELINE.md) and [`CHANGELOG.md`](CHANGELOG.md).

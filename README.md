...
**Current application baseline:** v1.55.4.1
...
The v1.55 map-engine work is a staged migration from Leaflet to OpenLayers. **v1.55.4** established the parity and architecture baseline; **v1.55.4.1** is its first live-parity correction. OpenLayers remains opt-in with `?mapEngine=openlayers`, but the current editable renderer, rendered hit-testing, selection-highlight refresh, references, GIS services and transient/edit overlays now sit behind the `EditPolygonMap` contract. The live acceptance pass exposed a true-circle projection mismatch: the draw preview was a projected visual circle while the committed renderer used the geographic export tessellation, causing large southern-hemisphere circles to shift visually and distort after the second click. v1.55.4.1 separates map-display materialisation from export materialisation so the committed screen-mode circle exactly matches the preview while GML/GeoJSON interoperability retains canonical metre-radius semantics. Leaflet remains the default/reference engine only until the deployed parity checklist passes. See [`ARCHITECTURE.md`](ARCHITECTURE.md), [`QUALITY_BASELINE.md`](QUALITY_BASELINE.md) and [`CHANGELOG.md`](CHANGELOG.md).

...
**Current application baseline:** v1.56.0.3
...
The v1.56.0.3 foundation ships with:
...
The v1.56.0.3 overlay implementation deliberately migrates the existing Turf behaviour into the new framework. **v1.56.1** is the robust topology phase: GEOS-backed Difference, Symmetric difference, true two-layer Union, dissolve by attribute and Make Valid.
...
This runs repository integration checks, the v1.56.0.3 single-runtime/Processing-Toolbox audit, the binding/architecture no-growth audit and the JavaScript test suite.
...
The map-runtime migration and its single-runtime cleanup are complete. **v1.56.0.3 is the Processing Toolbox foundation release**, built on the stable v1.55.7.4 map/drawing baseline.

1. **v1.56 — Processing Toolbox:** v1.56.0.3 establishes the registry/core/worker/UI and first eight tools; v1.56.1–v1.56.3 add robust GEOS topology, conversion/maintenance and spatial-analysis tools.

...
**Current application baseline:** v1.55.1.11
...
The v1.55 map-engine work is a staged migration from Leaflet to OpenLayers. v1.55.0 introduced the engine-neutral `EditPolygonMapAdapter` contract without changing the renderer. v1.55.1 adds the first real **OpenLayers parity runtime** behind `?mapEngine=openlayers`: OpenLayers owns the map view, navigation, built-in basemaps, editable vector display, and XYZ/TMS/WMS GIS service layers while Leaflet remains the normal default and temporarily provides a synchronised transparent compatibility surface for specialised overlays that have not yet been ported. Project geometry remains canonical WGS84 GeoJSON and there are no project-format, history, schema, CRS or analysis-model changes. v1.55.1.2 adds live rendered-geometry feedback during vertex dragging while keeping each completed drag as one undoable edit. v1.55.1.3 moved geometry previews, Geometry Health locate/repair overlays, location-search markers, measurements, selected-line endpoints, editable point handles and true-circle edit handles onto the engine-neutral map runtime; in OpenLayers mode those paths are now native OpenLayers/DOM overlays instead of the Leaflet compatibility surface, and the temporary parser-blocking `document.write()` loader was removed. v1.55.1.4 fixed the first live-test regressions from that move: OpenLayers DOM handles/measurement labels use a dedicated pane above the map canvas so true-circle controls and measurement values remain visible. v1.55.1.5 introduced the unified map-selection model. v1.55.1.6 hardened that parity against live OpenLayers behaviour by using rendered-feature hit detection and separating saved-measurement selection from editing. v1.55.1.8 completed the measurement editing/history and vector-cache pan fixes. v1.55.1.9 fixes the remaining true-circle ordinary-click failure at the event-delivery layer: OpenLayers selection now has a map-container fallback for clicks intercepted by the temporary Leaflet compatibility surface, legacy Leaflet editable circles are non-interactive whenever OpenLayers is active, and the compatibility CSS no longer re-enables pointer events for all legacy interactive vectors. v1.55.1.10 fixes the deeper selection inconsistency found in live testing: OpenLayers ordinary click selection now tests true circles against the exact materialised polygon geometry passed to the OpenLayers renderer, matching rectangle/polygon/lasso selection instead of using a separate centre/radius screen calculation. v1.55.1.11 fixes the actual runtime blocker discovered after full source-order tracing: a legacy v1.16 LineString compatibility block was overwriting the modern geometry-aware `featuresAtLatLng()` selector later in the monolithic script, so all newer true-circle click fixes were silently replaced before the user clicked the map. The stale reassignment is removed and regression tests now assert the final runtime selector binding is never overwritten. Leaflet is now retained only for the remaining reference/image compatibility paths during parity testing.

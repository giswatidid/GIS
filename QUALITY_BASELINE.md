...
**Current baseline: v1.56.0.3.** OpenLayers remains the sole map runtime. The accepted v1.55.7.4 drawing/rendering baseline is unchanged; v1.56.0.3 keeps the accepted Processing Toolbox architecture and hardens `.epz` project-open history isolation so every reopened project starts with a clean undo/redo session.
...
- v1.56.0.3 executes Toolbox geometry in canonical WGS 84/Turf; robust GEOS-backed projected topology is deliberately deferred to v1.56.1.
...
The v1.56.0.3 no-growth ceilings are:
...
## Live parity evidence carried into v1.56.0.3
...
Live-test v1.56.0.3 Toolbox behaviour, then proceed to **v1.56.1 robust topology**: extend the existing GEOS-WASM adapter for Difference, Symmetric difference, true two-layer Union, dissolve by attribute and Make Valid without duplicating the Toolbox lifecycle.

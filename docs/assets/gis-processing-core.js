...
function preflight(requestValue,{layers=[],selectionIds=[]}={}){
  ...
  if(tool?.crsPolicy==='canonical'&&sourceFamilies.includes('polygon')&&sourceFeatures.length>5000)warnings.push('This is a large topology job. v1.56.0.4 uses the browser geometry engine in canonical WGS 84 coordinates; robust GEOS-backed projected overlay arrives in v1.56.1.');
  if(tool?.id==='intersection')warnings.push('v1.56.0.4 Intersection retains source attributes only. Overlay attribute combination is planned for v1.56.1.');
  if(tool?.id==='dissolve')warnings.push('v1.56.0.4 Dissolve combines all scoped polygons into one geometry. Dissolve by attribute is planned for v1.56.1.');

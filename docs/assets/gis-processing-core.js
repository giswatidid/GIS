...
function preflight(requestValue,{layers=[],selectionIds=[]}={}){
  ...
  for(const definition of inputDefinitions(tool)){
    ...
    const features=scopeFeatures(layer,config.scope,selectionIds),families=geometryFamilies(layer,config.scope,selectionIds);inputFeatures[definition.id]=features;inputFamilies[definition.id]=families;counts[definition.id]=features.length;
    if(!features.length)errors.push(config.scope==='selected'?`No features from ${layer.name||definition.label} are selected.`:config.scope==='filtered'?`The active filter on ${layer.name||definition.label} contains no features.`:`${layer.name||definition.label} contains no features.`);
    const allowed=definition.families||['point','line','polygon'],unsupported=families.filter(value=>!allowed.includes(value));if(unsupported.length)errors.push(`${tool?.title||'This tool'} does not support ${unsupported.join(', ')} geometry in ${definition.label||definition.id}.`);

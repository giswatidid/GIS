...
function joinCompletionMessage(output){
  const join=output?.join||{},diagnostics=join.diagnostics||{},operation=join.operation||'derived_result';
  const count=Number(output?.features?.length??output?.count??0);
  const noun=operation==='group_summary'?'summary rows':'records';
  const details=[];
  if(operation==='attribute_join'||operation==='spatial_join'){
    if(Number.isFinite(diagnostics.matchedTargets))details.push(`${Number(diagnostics.matchedTargets).toLocaleString()} matched`);
    if(Number.isFinite(diagnostics.unmatchedTargets))details.push(`${Number(diagnostics.unmatchedTargets).toLocaleString()} unmatched`);
    if(operation==='spatial_join'&&Number.isFinite(diagnostics.totalMatches))details.push(`${Number(diagnostics.totalMatches).toLocaleString()} spatial matches`);
  }else if(operation==='group_summary'&&Number.isFinite(diagnostics.groupCount)&&Number(diagnostics.groupCount)!==count){
    details.push(`${Number(diagnostics.groupCount).toLocaleString()} groups`);
  }
  return `Created “${output?.name||'Result'}”: ${count.toLocaleString()} ${noun}${details.length?` · ${details.join(' · ')}`:''}.`;
}

...
function selectionView(layer){
  ...
  <section class="gis-tool-card"><h3>Select on map</h3><p>Rectangle selection is drag-based. Polygon selection uses map clicks and a Finish control.</p><div class="gis-form-grid"><label>How to apply<select id="gisMapSelectionMode">${selectionModeOptions()}</select></label><div class="gis-button-field"><span>Selection shape</span><div><button class="primary" data-action="map-select-rectangle">Rectangle</button><button data-action="map-select-polygon">Polygon</button></div></div></div></section>

...
This document describes the current application architecture as of **v1.56.0.3**.
...
The worker operates on cloned canonical WGS 84 GeoJSON in v1.56.0.3. `gisProcessingCrs` and provenance therefore record `EPSG:4326` rather than claiming a projected CRS that was not actually used. Buffer uses Turf's geodesic distance handling; robust projected/GEOS topology is a v1.56.1 concern.
...
The end of `editpolygon-app.js` contains the **v1.56.0.3 runtime-authority boundary**. No later feature monkey-patches may be appended after that point.
...
The application remains a large historically layered file. v1.56.0.3 audits currently allow at most:
...
v1.56.0.3 establishes the **Processing Toolbox foundation**. The next processing step is v1.56.1 robust GEOS-backed topology, followed by geometry conversion/maintenance and spatial-analysis phases. Large-dataset virtualisation remains the v1.57 performance milestone.
